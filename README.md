...
modifies postgis from kartoza/postgis
Visit their page on the docker hub at: https://hub.docker.com/r/kartoza/postgis/

It modifies the kartoza/postgis container by several things:

# configs in .env-file

Put the docker-compose.yml and the .env -file into a directory.

Modify the .env -file by adding the according folders on your host machine and a password.

### first use

If you start the container for the first time, modify the docker-compose.yml
...
This will create the database cluster and create the config files.


### start existing database

comment the entrypoint again, so that the default entry point ``/start-postgresql.sh`` is used:
```
#entrypoint: /docker-entrypoint.sh
```


# add kmeans and pg_repack with the pgxnclient

It installes pgxnclient as a python packages and installes the following extensions:

* kmeans : for cluster analysis
* pg_repack

And it installes the plpython language.

The extension ``pgrouting`` is already intalled in the base container ```kartooza/postgis```
To activate these extensions and languages, execute:
...
The config dir is mapped to the folder ${LOCAL_CONFIG_DIR} (defined in the .env-file).
It is made persistent here.

You can modify the postgresql.conf and the pg_hba.conf file.

If you run the entrypoint: /docker-entrypoint.sh
it appends each time some parameters to the end of the files postgresql.conf and pg_hba.conf

This might be not desired. 


# More info on the kartoza/postgis containers
